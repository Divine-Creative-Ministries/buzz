...
mod track_c_tests {
    ...
    async fn pushed_context(
        ...
        run_test_git(&source, &["init", "--quiet", "--initial-branch=main"]).await;
        run_test_git(&source, &["config", "user.email", "finalize@test"]).await;
        run_test_git(&source, &["config", "user.name", "finalize"]).await;
        tokio::fs::write(source.join("file.txt"), b"committed\n")
            .await
            .expect("write source file");
        run_test_git(&source, &["add", "file.txt"]).await;
        run_test_git(&source, &["commit", "--quiet", "-m", "committed"]).await;
        let remote = hydrated.path().to_str().expect("hydrated path utf8");
        run_test_git(&source, &["push", "--quiet", remote, "main"]).await;
