...
grep -q '^BUNDLE_IDENTIFIER = org\.divinecreative\.buzz\.feature-work-1$' "$ios" \
...
grep -q '^APP_DISPLAY_NAME = DCM Buzz (Fix_Thing-2)$' "$ios" \
...
grep -Eq "^APP_DISPLAY_NAME = DCM Buzz \([A-Za-z0-9._-]+\)$" "$ios" \
...
grep -q '^BUNDLE_IDENTIFIER = org\.divinecreative\.buzz$' "$release_xcconfig" \
  && pass "Release.xcconfig keeps the production bundle identifier" \
  || fail "Release.xcconfig must keep BUNDLE_IDENTIFIER = org.divinecreative.buzz"
grep -q '^APP_DISPLAY_NAME = DCM Buzz$' "$release_xcconfig" \
  && pass "Release.xcconfig keeps the production display name" \
  || fail "Release.xcconfig must keep APP_DISPLAY_NAME = DCM Buzz"
...
grep -q 'resValue("string", "app_name", "DCM Buzz")' "$gradle" \
  && pass "Gradle default app_name stays DCM Buzz" \
  || fail "Gradle must declare the DCM Buzz app_name resValue"
...
if [[ "$3 $4 $5" == "shell pm list" ]]; then
  printf 'package:org.divinecreative.buzz\n'
  printf 'package:org.divinecreative.buzz.feature_work_1\n'
  printf 'package:org.divinecreative.buzz.w_2fast\n'
